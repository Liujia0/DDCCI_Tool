...
- **Raw I2C Runtime**: `i2c_dev.dll` (embedded as RC resource, extracted at runtime)
...
Output: `bin/x64/Release/DDCCI_Tool.exe`（单文件分发；运行时自动提取 `web/` 资源和 `i2c_dev.dll`）
...
| `ExtractWebResources(HINSTANCE)` | Extracts embedded web resources to `%LOCALAPPDATA%\DDCCI_Tool\web\` and extracts/loads `i2c_dev.dll` to `%LOCALAPPDATA%\DDCCI_Tool\` (or uses files next to exe in dev mode) |
...
Raw DDC/CI transport over embedded `i2c_dev.dll`.
...
- `i2c_dev.dll` 也由 exe 内部资源提取到 `%LOCALAPPDATA%\DDCCI_Tool\`
...
2. **~~单文件打包发布~~（已完成）** — Release 构建输出单个 `DDCCI_Tool.exe`，无额外伴随文件：
   - CRT 静态链接（/MT），WebView2Loader 静态链接，`web/` 与 `i2c_dev.dll` 均嵌入为 RC 资源（WEBRES）
